...
@app.post("/tickets",status_code = 201,response_model=TicketResponse) #200 - ok , 201 - resource
...
def ticket_read_by_id(id : str):
    if not ObjectId.is_valid(id):
        raise HTTPException(detail = "Invalid Ticket ID",status_code=403) #403 = forbidden
    doc = ticket_collection.find_one({"_id" : ObjectId(id)})
    if not doc:
        raise HTTPException(detail = "Ticket Not Found",status_code=404) #404 = Not found
